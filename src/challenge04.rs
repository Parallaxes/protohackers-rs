...
async fn handle_init(stream: TcpStream, _tx: Arc<broadcast::Sender<Message>>) -> Result<Client> {
    ...
    writer_half.write_all(b"Welcome to Nyx! Please enter your username:\n").await?;
    ...
    loop {
        ...
        if username.is_empty() {
            writer_half.write_all(b"Username cannot be empty. Disconnecting client.").await?;
            return Err(Error::new(ErrorKind::InvalidInput, "Username must contain chars"));
        } else if username.len() > 32 {
            writer_half.write_all(b"Username too long (max 32 chars). Disconnecting client.").await?;
            return Err(Error::new(ErrorKind::InvalidInput, "Username is too long"));
        } else if !username.chars().all(char::is_alphanumeric) {
            writer_half.write_all(b"Username cannot contain prohibited chars. Disconnecting client.").await?;
            return Err(Error::new(ErrorKind::InvalidInput, "Username contains prohibited chars"));
        } else {
